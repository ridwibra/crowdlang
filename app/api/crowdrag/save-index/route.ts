  ...
  try {
    ...
    if (!allowedRoles.includes(session.user.role)) {
      return NextResponse.json(
        {
          message:
            "You do not have permission to save the chatbot index.",
        },
        {
          status: 403,
        },
    ...
    const items = body.items.map(
      (item: any) => {
        if (
          ...
          throw new Error(
            "Invalid RAG index item received.",
          );
        }
        ...
        if (!text) {
          throw new Error(
            "RAG index item contains empty text.",
          );
        }
    ...
    /*
     * Replace the existing index atomically at the
     * application level.
     */
