  ...
  try {
    ...
    for (const language of languages as any[]) {
      const countries = Array.isArray(language.countries)
        ? language.countries.filter(Boolean).join(", ")
        : "";

      documents.push({
        sourceType: "language",
        sourceId: language._id.toString(),
        text: `Language: ${language.name ?? "Unnamed language"}. Countries: ${countries}`,
    ...
    for (const reel of reels as any[]) {
      documents.push({
        ...
        text: `Reel (${reel.language ?? ""}): ${reel.caption ?? ""}. Transcription: ${reel.transcription ?? ""}`,
    ...
    for (const table of tables as any[]) {
      documents.push({
        ...
        text: `Table (${table.textType ?? "entry"}): ${table.text ?? ""} => ${table.translation ?? ""}`,
      ...
      if (table.text && table.translation) {
        documents.push({
          ...
          sourceId: table._id.toString(),
          text: `Reverse (${table.textType ?? "entry"}): ${table.translation} => ${table.text}`,
          language: table.language?.toString() ?? "",
        });
      }
    }
    ...
    for (const language of languages as any[]) {
      ...
      if (!hasContent) {
        ...
        documents.push({
          ...
          sourceId: languageId,
          text: `Basic information about ${language.name ?? "this language"}. Countries or territories: ${countries}.`,
          language: language.name ?? "",
        });
      }
    }
    ...
    console.error("Could not prepare RAG documents:", error);
